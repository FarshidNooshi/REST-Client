...
public class HTTpService {
    ...
    void runService() throws Exception {
        ...
        try {
            ...
            while (connection.getResponseCode() / 100 == 3 && request.getMp().get("f").equals("true")) {
                ...
                out.println("redirected to " + location);
                connection = (HttpURLConnection) (new URL(location)).openConnection();
            }
            String ret = getResponse(connection);
            out.println(connection.getResponseCode() + " " + connection.getResponseMessage());
            out.println(ret);
    ...
    private void PrintHelp() {
        out.println("--url: [Address] := address as the request url");
        out.println("-M(--method): (GET, POST, PATCH, DELETE, PUT) := request method type");
        out.println("-i: tells to print the response headers.");
        out.println("-H(--headers): key:value&... := setting the headers of the request");
        out.println("-h(--help)  will print the help");
        out.println("-f := if typed it will allow the following redirection");
        out.println("--save [folder name] := will save the request for you");
        out.println("--data [message body] := is the preferred message body with formData");
        out.println("--json [message body] := is the preferred message body with Json");
        out.println("note that in each request you should use one of json and data");
        out.println("--upload [URI] := will upload a request from a URI in system, URI is a absolute path.");
        out.println("output [empty or name] := will save the response body in a txt file with name : output [name or current date]");
        out.println("--type := encoded is for urlencoded, formData is for FormData, binary is for uploading a binary upload, json is for json.");
        out.println("note that for encoded and formData the data should be in \"a=b&c=d\" and for json it should be like : {a:b,c:d} also for binary upload use:" + "--uploadBinary [address] to send it.");
    }
}